...
async def get_word_count(keyword: str, platform:str=None):
    if platform:
        ...
        result = pd.DataFrame({'keyword': [keyword], 'count': [count], 'platform': [platform]})
    else:
        count = df['title'].str.count(keyword).sum()
        result = pd.DataFrame({'keyword': [keyword], 'count': [count], 'platform': ['All']})
...
async def get_longest(platform:str, duration_type:str, year:int):
    ...
    result = pd.DataFrame({'title': [title], 'duration': [duration], 'type': [type_],'platform':[platform],'duration_type':[duration_type],'release_year':[year]})
...
la duración y el tipo de la película o serie de esa fila y se devuelve esa información.
"""
